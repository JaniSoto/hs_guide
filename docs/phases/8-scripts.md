...
!!! tip "How the Scripts Handle KMS/DRM for Sunshine"
    For Sunshine to capture a physical display perfectly, it needs access to the kernel's DRM (Direct Rendering Manager) via KMS. These scripts are designed to gracefully stop active sessions, wipe the DRM locks, and restart the `sddm` display manager so the GPU is freshly initialized and ready for Sunshine to capture `seat0`.

## 1. Write `start-kde.sh`

This script launches KDE Plasma on the physical display.
...
```
Make it executable: `sudo chmod +x /usr/local/bin/start-kde.sh`

## 2. Write `start-gaming.sh`

This script launches the Steam Gaming Mode (gamescope).
...
```
Make it executable: `sudo chmod +x /usr/local/bin/start-gaming.sh`

## 3. Write `stop-session.sh`

This script gracefully terminates whichever session is active and returns the server to its headless, idle state.
...
```
Make it executable: `sudo chmod +x /usr/local/bin/stop-session.sh`

## 4. Configure Passwordless Sudo

To make remote streaming totally seamless, we allow your user to run these specific scripts without typing a password every time.
